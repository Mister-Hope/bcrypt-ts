...
describe(compare, () => {
  it("should return true for valid password-hash pairs", async () => {
    ...
    const $2ySalt = salt2.replace(/\$2b\$/u, "$2y$");
    const hash2 = await hash("world", $2ySalt);

    await expect(compare("hello", hash1)).resolves.toBe(true);
    await expect(compare("hello", hash2)).resolves.toBe(false);
    await expect(compare("world", hash1)).resolves.toBe(false);
    await expect(compare("world", hash2)).resolves.toBe(true);
  });
...
describe(compareSync, () => {
  it("should return true for valid password-hash pairs", () => {
    ...
    const salt2 = genSaltSync().replace(/\$2b\$/u, "$2y$");
